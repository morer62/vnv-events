...
$router->get(function () {
    ...
    if (PlatformDetector::isMobileApp()) {
        return TemplateResponse::renderInTemplates("web-only-feature.twig", [
            ...
            "message" => "To complete your venue or service setup, you need to configure your payment method. This feature is only available through our website for security reasons.",
            "icon" => "💳",
...
$router->post(function () {
    try {
        ...
        // Verificar si el usuario ya tiene al menos un SERVICE (nivel 3)
        $serviceRepo = new ServiceRepository();
        $hasService = !!$serviceRepo->getOne(["user_id" => $user->getId()]);

        $needsProfile = !$hasService;
        $nextTitle = $needsProfile ? "Create your vendor profile" : "";
        $base = rtrim($_ENV["APP_URL"] ?? '', '/');
        $nextUrl = $needsProfile ? ($base . "/panel/service/create") : "";
