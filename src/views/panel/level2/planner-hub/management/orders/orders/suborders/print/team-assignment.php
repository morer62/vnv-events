...
if ($user->getLevel() === 4) {
    ...
    $order = $orderRepo->getOne(["id" => $suborder->id_order]);
}
...
$manualTasks = $teamTaskRepo->getAllWithoutOwner(["id_suborder" => $suborderId, "id_service" => 0]);
...
foreach ($services as $assigned) {
    ...
    $noteEntry = $notesRepo->findBySuborderAndService($suborderId, $assigned["id_service"]);
