    ...
    try {
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    <h2>Hello {$name},</h2>
                    <p>Your account has been created as a <strong>{$userTypeText}</strong>. Here are your credentials:</p>
                    <div class='credentials'>
                        <p><strong>Email:</strong> {$email}</p>
                        <p><strong>Password:</strong> <code style='font-size: 16px; background: #f0f0f0; padding: 5px 10px; border-radius: 3px;'>{$password}</code></p>
                    </div>
        ...
        $result = EmailServiceFactory::sendWithOwnerProvider($ownerId, $email, $subject, $message, true);
...
$router->post(function () {
    ...
    if (!empty($_POST["link_existing_user_id"])) {
        ...
        $linkStoreTeamRole = trim((string)($_POST["link_store_team_role"] ?? 'general'));
        ...
        if ($success) {
            $storeUserRolesRepo->saveValidatedRole((int)$currentOwnerId, (int)$existingUserId, $linkStoreTeamRole);
    ...
    $storeTeamRole = trim((string)($_POST["store_team_role"] ?? 'general'));
