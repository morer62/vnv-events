...
$router->post(function () {
    ...
    if ($formType === "convert_to_client") {
        ...
        $storeUserRolesRepo->deleteByOwnerAndUser((int)$currentOwnerId, (int)$user->id);
    ...
    if ($formType === "convert_to_member") {
        ...
        $storeTeamRole = trim((string)($_POST["convert_store_team_role"] ?? 'general'));
        ...
        $storeUserRolesRepo->saveValidatedRole((int)$currentOwnerId, (int)$user->id, $storeTeamRole);
    ...
    if ($currentOwnerId && (int)$user->level === 4) {
        $current_store_team_role = $storeUserRolesRepo->getRoleValueByOwnerAndUser((int)$currentOwnerId, (int)$user->id);
    }
    ...
    $originalData = [
        ...
        "store_team_role" => $current_store_team_role
    ];
    ...
    if ($user->level == 4 && isset($_POST["store_team_role"])) {
        $newData["store_team_role"] = trim((string)$_POST["store_team_role"]);
    }
    ...
    unset($userUpdateData["store_team_role"]);
