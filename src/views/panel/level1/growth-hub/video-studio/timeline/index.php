...
$router->post(function(): JsonResponse {
    try{
        ...
        $result=(new AiTranscriptTimelineService())->apply($job,$edits,!empty($_POST['remove_long_pauses']),max(.6,min(10,(float)($_POST['pause_threshold']??1.25))),is_array($pauseEdits)?$pauseEdits:[],is_array($precisionEdits)?$precisionEdits:[],!empty($_POST['cut_only_mode']));
