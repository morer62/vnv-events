...
$router->get(function(){
    ...
    $db=new Connection();$db->query("SELECT COUNT(*) total,SUM(status='GENERATED') generated,SUM(status='PUBLISHED') published FROM cms_contents WHERE id_owner=:owner");$db->bind(':owner',$owner);$content=$db->fetchOne();
    $db->query("SELECT COUNT(*) total,SUM(status IN ('READY','COMPLETED')) ready,SUM(status IN ('QUEUED','RENDERING')) processing FROM ai_agent_media_jobs WHERE id_owner=:owner");$db->bind(':owner',$owner);$media=$db->fetchOne();
