...
$router=new Router();
$router->get(function(){
    $s=LoginService::getSession();$db=new Connection();$db->query("SELECT * FROM ai_agent_editorial_plans WHERE id_owner=:owner AND site_key=:site ORDER BY id LIMIT 1");$db->bind(':owner',(int)$s->getOwner());$db->bind(':site',SiteContext::siteKey());$plan=$db->fetchOne();
...
$router->post(function(){
    ...
    try{
        ...
        $db->query("INSERT INTO ai_agent_editorial_plans(id_owner,site_key,id_user,articles_per_week,location_pages_per_week,pages_per_week,social_posts_per_week,video_posts_per_week,instructions)
          VALUES(:owner,:site,:user,:articles,:locations,:pages,:social,:videos,:instructions)
        ...
        foreach(['owner'=>$owner,'site'=>SiteContext::siteKey(),'user'=>$user]+$values as $k=>$v)$db->bind(':'.$k,$v);$db->execute();
        if(($_POST['action']??'save')==='generate'){
            $db->query("SELECT id,title,content_type,status,updated_at FROM cms_contents WHERE id_owner=:owner AND site_key=:site ORDER BY updated_at DESC LIMIT 100");$db->bind(':owner',$owner);$db->bind(':site',SiteContext::siteKey());$library=$db->fetchAll();
            ...
            $db->query("UPDATE ai_agent_editorial_plans SET last_planned_at=NOW() WHERE id_owner=:owner AND site_key=:site");$db->bind(':owner',$owner);$db->bind(':site',SiteContext::siteKey());$db->execute();MessageUtil::setMessage('Weekly plan generated for approval.');LocationUtils::redirectInternal('panel/agents/approval?id='.$id);
        }
