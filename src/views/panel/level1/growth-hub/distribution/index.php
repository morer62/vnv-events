...
$router->get(function(){
    ...
    $db=new Connection();$db->query("SELECT id,title,status,featured_image_url,excerpt,body_html,canonical_url FROM cms_contents WHERE id_owner=:owner AND status IN ('GENERATED','PUBLISHED') ORDER BY updated_at DESC LIMIT 100");$db->bind(':owner',$owner);$contents=$db->fetchAll();
    ...
    $db->query("SELECT p.*,a.agent_key FROM ai_agent_approvals p JOIN ai_agents a ON a.id=p.id_agent WHERE p.id_owner=:owner AND p.action_type IN ('PUBLISH_CAROUSEL','PUBLISH_SOCIAL_CREATIVE','REVIEW_SHORT_VIDEO') ORDER BY p.id DESC LIMIT 100");$db->bind(':owner',$owner);$creativeLibrary=$db->fetchAll();foreach($creativeLibrary as $creative){$payload=json_decode((string)$creative->payload_json,true)?:[];$creative->preview_url=(string)($payload['image_url']??($payload['slides'][0]['image_url']??''));$creative->preview_caption=mb_substr((string)($payload['caption']??''),0,240);$creative->artwork_status=(string)($payload['image_generation']['status']??'');$creative->artwork_error=(string)($payload['image_generation']['error']??'');}
