...
$router->get(function(){
    ...
    $db->query("SELECT id,title,status,output_url,created_at FROM ai_agent_media_jobs WHERE id_owner=:owner AND status IN ('READY','COMPLETED') ORDER BY id DESC LIMIT 100");$db->bind(':owner',$owner);$media=$db->fetchAll();
    ...
    return TemplateResponse::render(__DIR__.'/index.twig',['mode'=>$mode,'agent'=>$agent,'contents'=>$contents,'media'=>$media,'connections'=>$connections,'approvals'=>$repo->pendingApprovals($owner,(int)$agent->id)]);
});
