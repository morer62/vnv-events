...
$router->post(function () {
    ...
    $route = $manualRoute !== '' ? $routesRepository->normalizeRoute($manualRoute) : $routesRepository->normalizeRoute('blog/' . $slug);
    ...
    if ($contentsRepository->slugExists($slug, $ownerId, $post->language ?? 'en', $id)) {
        $errors[] = "That slug already exists.";
    }

    if ($mainRoute) {
        if ($routesRepository->routeExists($route, $ownerId, $post->language ?? 'en', (int)$mainRoute->id)) {
            $errors[] = "That public route already exists.";
        }
    } else {
        if ($routesRepository->routeExists($route, $ownerId, $post->language ?? 'en')) {
            $errors[] = "That public route already exists.";
        }
    }
