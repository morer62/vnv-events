...
$router->post(function () {
    ...
    // Manager availability is operational information only. It must never
    // prevent a customer from paying through public Order Access.
    try {
        ...
        if (($availability['status'] ?? '') !== ManagerAvailabilityService::AVAILABLE) {
            $availabilityEngine->notifyLevelOne((int)$order->id_owner, 'Manager scheduling review required for order #'.$order->id.'. Customer payment remains available.', 'panel/planner-hub/management/orders/orders/edit?id='.$order->id);
        }
        ...
        // Payment intentionally continues if the operational scheduling check
        // is unavailable. Level 1 can review the assignment independently.
        error_log('[ManagerAvailability] Non-blocking first-payment check failed for order '.$orderId.': '.$e->getMessage());
    }
