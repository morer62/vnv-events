...
final class AiTranscriptTimelineService
{
    ...
    private function applyCommandToPlan(array &$plan,array $command): void
    {
        ...
        if($type==='caption'){$duration=null;$size=75;$spacing=2.0;$animation='word';if(preg_match('/duration\s*:\s*(\d+(?:\.\d+)?)\s*s/i',$options,$m))$duration=max(.5,min(3600,(float)$m[1]));if(preg_match('/size\s*:\s*(\d{2,3})\s*%/i',$options,$m))$size=max(35,min(140,(int)$m[1]));if(preg_match('/letter_spacing\s*:\s*(-?\d+(?:\.\d+)?)\s*px/i',$options,$m))$spacing=max(-2,min(12,(float)$m[1]));if(preg_match('/animation\s*:\s*(word|phrase|static)/i',$options,$m))$animation=strtolower($m[1]);$plan['caption_style_events'][]=['start'=>$at,'end'=>$duration===null?null:$at+$duration,'preset'=>AiCaptionStyleRegistry::find((string)$command['instruction'])['id'],'size_percent'=>$size,'letter_spacing'=>$spacing,'animation'=>$animation,'source'=>'timeline_command'];}
        ...
        elseif($type==='transition'){$normalized=str_replace([' ','-'],'_',$instruction);if($normalized==='whip')$normalized='whip_left';$allowed=['blur','dip_to_black','flash','whip_left','whip_right','soft_fade'];$effect=in_array($normalized,$allowed,true)?$normalized:'blur';$duration=in_array($effect,['dip_to_black','soft_fade'],true)?.38:.24;if(preg_match('/duration\s*:\s*(\d+(?:\.\d+)?)\s*s/i',$options,$m))$duration=max(.12,min(.8,(float)$m[1]));$plan['transitions'][]=['timestamp'=>$at,'type'=>$effect,'duration'=>$duration,'reason'=>$command['instruction'].' '.$options,'source'=>'timeline_command'];}
        elseif($type==='zoom'){$direction=mb_strtolower($instruction.' '.$options);$focus=str_contains($direction,'left')?.22:(str_contains($direction,'right')?.78:.5);$zoom=1.10;if(preg_match('/scale\s*:\s*(\d+(?:\.\d+)?)\s*%?/i',$options,$m))$zoom=max(1.02,min(1.40,(float)$m[1]/100));$duration=str_contains($options,'persistent')?3600:3;if(preg_match('/duration\s*:\s*(\d+(?:\.\d+)?)\s*s/i',$options,$m))$duration=max(.5,min(3600,(float)$m[1]));$plan['camera_moves'][]=['start'=>$at,'end'=>$at+$duration,'type'=>'reframe','zoom'=>$zoom,'focus_x'=>$focus,'focus_y'=>.42,'reason'=>$command['instruction'].' '.$options,'source'=>'timeline_command'];}
