...
class AiAgentsRepository
{
    ...
    public function allForOwner(int $ownerId): array
    {
        $this->db->query("SELECT a.*,
            ...
            FROM ai_agents a WHERE a.id_owner = :owner ORDER BY a.category, a.name");
    ...
    {
        $this->db->query("SELECT * FROM ai_agents WHERE id_owner = :owner AND agent_key = :agent_key LIMIT 1");
        $this->db->bind(':owner', $ownerId);
    ...
    public function seed(int $ownerId, array $definitions): void
    {
        foreach ($definitions as $key => $definition) {
            $this->db->query("INSERT INTO ai_agents
                (id_owner, agent_key, name, description, category, status, approval_mode, settings_json)
                VALUES (:owner, :agent_key, :name, :description, :category, :status, 'ALWAYS', :settings)
    ...
    public function createRun(int $agentId, int $ownerId, string $trigger, ?int $userId, array $input = []): int
    {
        $this->db->query("INSERT INTO ai_agent_runs
            (id_agent, id_owner, trigger_type, status, input_json, created_by, started_at)
            VALUES (:agent, :owner, :trigger, 'RUNNING', :input, :user, NOW())");
    ...
    public function pendingApprovals(int $ownerId, ?int $agentId = null): array
    {
        $sql = "SELECT p.*, a.name AS agent_name, a.agent_key FROM ai_agent_approvals p
            JOIN ai_agents a ON a.id = p.id_agent WHERE p.id_owner = :owner";
        if ($agentId) {
    ...
    public function approvalInbox(int $ownerId,string $tab='all',string $status='PENDING',string $search='',int $page=1,int $perPage=30): array
    {
        ...
        $sql="SELECT p.*,a.name agent_name,a.agent_key FROM ai_agent_approvals p JOIN ai_agents a ON a.id=p.id_agent WHERE p.id_owner=:owner AND p.status=:status";
        ...
        $sql.=" ORDER BY p.id DESC LIMIT {$perPage} OFFSET {$offset}";$this->db->query($sql);$this->db->bind(':owner',$ownerId);$this->db->bind(':status',$status);if($search!=='')$this->db->bind(':search','%'.$search.'%');return $this->db->fetchAll();
    }
    ...
    public function approvalInboxTotal(int $ownerId,string $tab='all',string $status='PENDING',string $search=''): int
    {
        ...
        $search=trim($search);$sql="SELECT COUNT(*) total FROM ai_agent_approvals p JOIN ai_agents a ON a.id=p.id_agent WHERE p.id_owner=:owner AND p.status=:status";
        if(isset($groups[$tab])){$quoted=implode(',',array_map(fn($v)=>"'".str_replace("'","''",$v)."'",$groups[$tab]));$sql.=" AND p.action_type IN ({$quoted})";}
        if($search!=='')$sql.=" AND (p.title LIKE :search OR a.name LIKE :search OR p.action_type LIKE :search)";
        $this->db->query($sql);$this->db->bind(':owner',$ownerId);$this->db->bind(':status',$status);if($search!=='')$this->db->bind(':search','%'.$search.'%');$row=$this->db->fetchOne();return (int)($row->total??0);
    }
    ...
    public function approvalCounts(int $ownerId): array
    {
        $this->db->query("SELECT
          ...
          FROM ai_agent_approvals WHERE id_owner=:owner");$this->db->bind(':owner',$ownerId);$row=$this->db->fetchOne();
    ...
    public function findApproval(int $ownerId, int $approvalId): ?object
    {
        $this->db->query("SELECT p.*,a.agent_key,a.name AS agent_name,a.description AS agent_description
            ...
            WHERE p.id=:id AND p.id_owner=:owner LIMIT 1");
        $this->db->bind(':id',$approvalId); $this->db->bind(':owner',$ownerId);
        return $this->db->fetchOne() ?: null;
    }

    public function approvalHistory(int $ownerId, int $runId, string $actionType): array
    {
        $this->db->query("SELECT * FROM ai_agent_approvals WHERE id_owner=:owner AND id_run=:run
            AND action_type=:action ORDER BY id DESC");
        $this->db->bind(':owner',$ownerId); $this->db->bind(':run',$runId); $this->db->bind(':action',$actionType);
        return $this->db->fetchAll();
    }
    ...
    public function requestApprovalRevision(int $ownerId, object $approval, int $reviewerId, string $note, array $revisedPayload): int
    {
        $this->db->query("UPDATE ai_agent_approvals SET status='REVISION_REQUESTED',reviewed_by=:reviewer,
            review_note=:note,reviewed_at=NOW() WHERE id=:id AND id_owner=:owner AND status='PENDING'");
        $this->db->bind(':reviewer',$reviewerId); $this->db->bind(':note',$note);
        $this->db->bind(':id',$approval->id); $this->db->bind(':owner',$ownerId); $this->db->execute();
    ...
    {
        $this->db->query("UPDATE ai_agent_approvals SET payload_json=:payload WHERE id=:id AND id_owner=:owner AND status='PENDING'");
        $this->db->bind(':payload',json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
        $this->db->bind(':id',$approvalId); $this->db->bind(':owner',$ownerId); $this->db->execute();
    }
    ...
    public function reviewApproval(int $ownerId, int $approvalId, string $status, int $reviewerId, string $note): void
    {
        ...
        $this->db->query("UPDATE ai_agent_approvals SET status = :status, reviewed_by = :reviewer,
            ...
            WHERE id = :id AND id_owner = :owner AND status = 'PENDING'");
        $this->db->bind(':status', $status);
        ...
        $this->db->query("UPDATE ai_agent_runs r
            ...
            WHERE decided.id=:approval AND decided.id_owner=:owner
        ...
        $this->db->bind(':approval',$approvalId);$this->db->bind(':owner',$ownerId);$this->db->execute();
    }
    ...
    public function rotateWebhook(int $ownerId, string $key): string
    {
        ...
        $this->db->query("UPDATE ai_agents SET webhook_token_hash = :hash, webhook_token_hint = :hint
            WHERE id_owner = :owner AND agent_key = :agent_key");
    ...
    public function updateConfiguration(int $ownerId, string $key, array $data): void
    {
        ...
        $this->db->bind(':next_run',$nextRun); $this->db->bind(':owner',$ownerId); $this->db->bind(':agent_key',$key);
        $this->db->execute();
    }

    public function dueScheduledAgents(): array
    {
        $this->db->query("SELECT * FROM ai_agents WHERE status='ACTIVE' AND schedule_enabled=1
